...
不需要噴嘴、糖、畫布也能做的事,而且都是有價值的實測:

1. **`sugar_test` step 0~1** —— 範圍檢查、方形轉角。確認手臂會動、行程夠、
   `ACCURACY` 設多少轉角才不會過圓。全程在示教的 `tsafe` 平面上,不往下降。

2. **`sugar_test` step 2** —— 短線段計時對照。**現階段最有價值的測試**,
   完全不需要治具。同樣 120mm 的直線,分別用 1 段 / 40 段 / 120 段走完,
...
執行前一定要:設 `TOOL 0`(沒裝工具時)、教一個四周淨空的 `tsafe` 點、
速度倍率調到 20% 以下、有人手按急停。

`sugar_calib` 要有實體平面當基準,還沒有畫布就先跳過。
